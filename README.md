...
**BumbleBees is fully WASM-compatible and runs in modern web browsers!**

#### Quick Build

```bash
# Automated build script (installs dependencies if needed)
./build-wasm.sh
...
```bash
# 1. Install wasm32 target (one-time setup)
...
**Output**: Generates `pkg/` directory with JavaScript bindings (~6.5KB + assets)

The game includes:
- `index.html` - Main web entry point
- `build-wasm.sh` - Automated build script
- `WASM-README.md` - Complete WASM documentation
...
├── pkg/                 # WASM output (generated by build-wasm.sh)
│   ├── ten.js           # JavaScript bindings
│   ├── ten_bg.wasm      # WebAssembly binary
│   └── ten.d.ts         # TypeScript definitions
├── Cargo.toml           # Rust dependencies and metadata
├── CLAUDE.md            # Developer documentation
├── AGENTS.md            # Build/lint/test commands reference
├── WASM-README.md       # WASM deployment guide
...
- [WASM-README.md](WASM-README.md) - Complete WASM/Web deployment guide
